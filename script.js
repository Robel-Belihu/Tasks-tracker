function submitIssue(e) {
  ...
  const isToBeDoneBy = getInputValue("isToBeDoneBy");
  const id = Math.floor(Math.random() * 100000000) + "";
  const status = "Open";

  if (description.length == 0 || isToBeDoneBy.length == 0) {
    ...
    const issue = { id, description, priority, isToBeDoneBy, status };
    let issues = [];
...
const fetchIssues = () => {
  ...
  for (var i = 0; i < issues.length; i++) {
    const { id, description, priority, isToBeDoneBy, status } = issues[i];
    ...
    issuesList.innerHTML += `<div class="well">
                                ...
                                <p><span class="glyphicon glyphicon-user"></span> ${isToBeDoneBy}</p>
